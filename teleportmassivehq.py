...
def display_message(role, message):
    print(f"Displaying message: {role}: {message}")
...
def check_for_new_messages():
    ...
    if thread_id:
        try:
            # retrieve the messages using the load initial messages function
...
def retrieve_thread_messages(thread_id):
    try:
        ...
        if thread_messages_response.data:
            print(f"Retrieved {len(thread_messages_response.data)} messages.")
